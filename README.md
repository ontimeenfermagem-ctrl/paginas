...
Aviso de inscrição ao n8n (Imersão GPS): cada inscrição **nova** da página `imersao-gps` sai num POST `inscricao` para `GPS_WEBHOOK_URL` (padrão `https://n8n.tecnicadevalor.com.br/webhook/gps-outubro`; `off` desliga) com `pagina` (`id`, `nome`, `produto`, `rota`, `url`), `inscricao_id`, `inscrito_em`, `lead` (`nome`, `primeiro_nome`, `whatsapp`, `whatsapp_digits`, `whatsapp_internacional`, `email`), `utm` (as 5), `sck` (o que foi no link da Hotmart), `rastreio` (`fbclid`, `gclid`, `page_url`, `referrer`, `dispositivo`), `checkout_url` e `enviado_em`. Uma vez por pessoa: reenviar o formulário soma clique, não aviso. Sai depois da resposta ao navegador (o checkout nunca espera o n8n), com as mesmas 3 tentativas da pesquisa; a entrega confirmada fica em `inscricoes.webhook_enviado_em`, e o que não chegou é reenviado pela varredura (mesma janela e ritmo). O endereço fica no servidor (`DEFAULT_WEBHOOKS_INSCRICAO` no `server.mjs`), e não no `checkout-config.js`, que é público. A Viver de Furo não tem webhook.
...
Abaixo, as compras recentes (cada aviso do produto da página, com o `sck` e o aviso de quantas compras aprovadas não casaram com nenhuma inscrição), a lista de inscritos com selo "comprou"/"não comprou", o `sck` que foi para a Hotmart e o link de WhatsApp, e o botão "Baixar CSV" (com a página e o período da tela). O link antigo `?pagina=inscricoes` (de quando havia uma aba só) abre a primeira. Com o SQL antigo no banco, o que ele não manda (mídia, conteúdo, vendas na Hotmart, `sck` dos avisos) some da tela e o resto funciona. A aba de obrigado traz (um cartão por página: nome, rota, perfis que caem nela, link do grupo ou o aviso "link do grupo ainda não configurado", o funil pesquisas concluídas atribuídas → chegaram à página → clicaram no grupo com a porcentagem e a base de cada passo, e a divisão por perfil, origem e dia). A aba "Perfis atualizados" (`GET /api/painel/perfis`) junta as duas origens de "contato + profissão" sem pesquisa completa: a página curta `/atualizacao-perfil` (convite no WhatsApp) e o lead da DM do Instagram, coletado pelo ManyChat (`POST /api/integrations/manychat/lead`, origem `instagram · manychat`). Ela lê a MESMA tabela da pesquisa, filtrada pela coluna `pesquisa` (`atualizacao-perfil`), então nada dela entra nos números do ICP — e vice-versa. Mostra quantas pessoas atualizaram o perfil no período, a profissão mais comum, uma aba por profissão com o total de cada uma (contadas com `count=exact`, sem trazer linha — inclusive o **grupo** `enfermagem`, que o ManyChat manda quando a DM não separa técnica de enfermeira) e a lista com nome, WhatsApp clicável, e-mail, a profissão (com o código interno no `title`) e a origem. O selo "Aviso pendente" é só das linhas da página curta: lead do Instagram não tem aviso ao n8n. Escolher uma profissão filtra só a lista: os quatro números continuam inteiros.

O período da barra vale para todas as abas; perfil e situação só existem na da pesquisa, e a busca vale também na de perfis. Página de inscrição nova = uma entrada em `PAGINAS` do `js/checkout-config.js` (e o `<rota>.html`, se ela morar aqui; a aba do painel nasce sozinha); outra página de funil = uma entrada no registro `PAGINAS` de `js/painel.js` + o bloco dela no `painel.html`.
